...
# Helper function to calculate resume similarity
def is_similar(selected_resume, resume):
    selected_skills = set(selected_resume.get('skills', []))
    selected_experience = selected_resume.get('experience') or 0
    selected_education = set(selected_resume.get('education', []))

    resume_skills = set(resume.get('skills', []))
    resume_experience = resume.get('experience') or 0
...
def fetch_similar_resumes(id):
    ...
    try:
        ...
        for resume in resumes_collection.find():
            ...
            if is_similar(selected_resume, resume):
                resume['_id'] = str(resume['_id'])  # Convert ObjectId to string
